...
select rn, skips
	, (select array_agg(0::bigint)
		from skips) as hits
	, 1::int as counter, skips as true_skips
from skips
where rn = 1
union all
select s.rn, s.skips
	, case when y.rn = 1
		then array_prepend(y.hits[1]+1,y.hits[2:s.num])
		when y.rn = num 
		then array_append(y.hits[1:s.num-1], y.hits[num]+1)
		else array_cat(array_append(y.hits[1:y.rn-1], y.hits[y.rn]+1), y.hits[y.rn+1:s.num])
		end as hits
	, y.counter+1 as counter
	, y.skips + y.hits[y.rn] as true_skips
from skippy y
inner join skips s
	on s.rn = y.rn + y.skips + y.hits[y.rn]
-- 	or (y.counter = 1 and s.rn = 1 and y.rn = 1)
where counter < 500000
)
select * from skippy
-- where counter <> 0
...
select rn, skips
	, (select array_agg(skips::bigint)
		from skips) as hits
	, 1::int as counter, skips as true_skips, num
from skips
where rn = 1
union all
