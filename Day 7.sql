...
gyxo (61)
cntj (57)'::text as inputs
;



drop table if exists tron;
create temp table tron as(
with splits as( select regexp_split_to_table(inputs,'\n') as trons
from temp_input
-- from test_temp_input
)

(
select (regexp_split_to_array((regexp_split_to_array(trons, ' -> '::text))[1],' '))[1] as lower_tron
	,(regexp_replace((regexp_split_to_array((regexp_split_to_array(trons, ' -> '::text))[1],' '))[2],'\(|\)','','g'))::int as lower_tron_weight
-- 	,(regexp_split_to_array(trons, ' -> '::text))[2] as higher_trons
	,regexp_split_to_table((regexp_split_to_array(trons, ' -> '::text))[2],', ') as higher_trons
from splits t
where (regexp_split_to_array(trons, ' -> '::text))[2] is not null
)
union
(
select (regexp_split_to_array((regexp_split_to_array(trons, ' -> '::text))[1],' '))[1] as lower_tron
	...
	,(regexp_split_to_array(trons, ' -> '::text))[2] as higher_trons
from splits t
where (regexp_split_to_array(trons, ' -> '::text))[2] is null
)
)
;

select distinct t.lower_tron
from tron t
left join tron t2
	on t2.higher_trons = t.lower_tron
where t2.lower_tron is null
;
...
select distinct on (t.higher_trons)
	t.lower_tron as tree_parent
	,array[t.lower_tron,t2.lower_tron]::text[] as path
	,t2.lower_tron as tron_name
-- 	,t2.lower_tron_weight as weight
-- 	,0::int as weight
	,1::int as counter
from tron t
left join tron t2
	on t.higher_trons = t2.lower_tron
where t.higher_trons is not null
union all
select distinct on(t2.lower_tron)
	tree_parent
	,array_append(path, t2.lower_tron) as path
	,t2.lower_tron as tron_name
-- 	,t2.lower_tron_weight as weight
	,counter + 1 as counter
from weights w
left join tron t
	on t.lower_tron = w.tron_name
inner join tron t2
	on t.higher_trons = t2.lower_tron
where counter < 10
)
...
drop table if exists tree_weight;
create temp table tree_weight as 
select path[1] as path
	,weight
	,count(1)
from(
select path[1:2] as path
	,sum(lower_tron_weight) as weight
from(
select distinct
	t.*
	,t2.lower_tron_weight
from trees t
left join tron t2
	on t2.lower_tron = t.tron_name
)t
group by 1
-- order by 1
)t2
...
drop table if exists bad_trees;
create temp table bad_trees as
select path as tree_parent
	,max(weight) - min(weight) as error
	,max(weight) as max_weight
	,min(weight) as min_weight
from tree_weight t3
group by 1
having max(weight) - min(weight) > 0
;




with so_close_to_done as(
select distinct on (tr.lower_tron)
	error
	,weight
	,records
	,lower_tron_weight
...
select error
	,path
	,weight
	,count(1) over(partition by weight) as records

from(
select distinct on (t.tree_parent)
	t.tree_parent
	,bt.min_weight
	,bt.max_weight
	,bt.error
	,bt2.tree_parent as not_right_row_flag
from trees t
inner join bad_trees bt
	using(tree_parent)
left join bad_trees bt2
	on bt2.tree_parent = any(t.path[2:])
order by t.tree_parent
	,bt2.tree_parent is not null desc
)a
left join (select path[1:2] as path
	,sum(lower_tron_weight) as weight
from(
select distinct
	t.*
...
group by 1) b
on b.path[1] = a.tree_parent
where not_right_row_flag is null
)c
left join tron tr
	on tr.lower_tron = path[2]
	and c.records = 1
)

select case when s.weight > s2.weight
	then s.lower_tron_weight - s.error
	when s.weight < s2.weight
	then s.lower_tron_weight + s.error
	end
from so_close_to_done s
left join so_close_to_done s2
	on s2.lower_tron_weight is null
where s.lower_tron_weight is not null
;