...
707	668	1778	1687	2073	1892	62	1139	908	78	1885	800	945	712	57	65'::text as inputs
;
...
select sum(row_max - row_min)
from(
select rn
	,min(entries::int) as row_min
	,max(entries::int) as row_max
from digits d
group by 1
)t
;
...
select d.rn
	,d.entries
	,d2.entries
	,d.entries / d2.entries as divide
from digits d
inner join digits d2
	on d2.rn = d.rn
	and d.entries % d2.entries = 0
	and d.entries <> d2.entries
)t
;