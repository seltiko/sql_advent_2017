...
x dec 368 if f >= 5212'::text as inputs
;
...
select row_number() over() as rn,
	inst,
	(regexp_matches(inst,'^(\w+) '))[1] as thing_to_change
	,(regexp_matches(inst,'^\w+ (\w+) '))[1] as direction_to_change
	,((regexp_matches(inst,'^\w+ \w+ (-?\d+)'))[1])::int as amount_to_change
	,(regexp_matches(inst,'^\w+ \w+ -?\d+ if (\w+) '))[1] as thing_to_check
	,((regexp_matches(inst,'^\w+ \w+ -?\d+ if \w+ (.{1,2}) '))[1]) as critera_to_check
	,((regexp_matches(inst,'^\w+ \w+ -?\d+ if \w+ .{1,2} (-?\d+)$'))[1])::int as amount_to_check
from splits t
...
select row_number() over() as kn
	,keys
	,0::int as val
from(
select thing_to_change as keys
from tron
union
select thing_to_check
from tron
order by keys
)t
...
with recursive steps (rn, vals) as(
select rn
	,case when critera_to_check = '>=' then
		case when vals[check_key_loc] >= amount_to_check
			then array_cat(
				array_append(vals[0:key_loc-1]
					,vals[key_loc] + amt_to_change_adj)
				,vals[key_loc+1:array_length(vals,1)])
			else vals
		end 
		when critera_to_check = '>' then
		case when vals[check_key_loc] > amount_to_check
			then array_cat(
				array_append(vals[0:key_loc-1]
					,vals[key_loc] + amt_to_change_adj)
...
select *
	,(select kn from register where t.thing_to_change = keys) as key_loc
	,(select kn from register where t.thing_to_check = keys) as check_key_loc
	,case when direction_to_change = 'dec' then amount_to_change::int * -1 else amount_to_change::int end as amt_to_change_adj
...
select t.*
	,(select kn from register where t.thing_to_change = keys) as key_loc
	,(select kn from register where t.thing_to_check = keys) as check_key_loc
	,case when t.direction_to_change = 'dec' then t.amount_to_change::int * -1 else t.amount_to_change::int end as amt_to_change_adj
	,s.vals
from steps s
inner join tron t
	on t.rn = s.rn + 1
...
select max(vals)
from(
select unnest(vals) as vals
from(
select *
from steps
--for part i just comment back in these two lines
-- order by rn desc 
-- limit 1
)t
)t2
