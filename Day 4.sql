...
jwfm ptjwrbl hhuv uolz adyweh qpj wxyogp igvnojq jmfw pqs fsnirby'::text as inputs
;


-- part ii
with words as(
select row_number() over(partition by rn) as wn
	,rn
	,unnest(entries) as letters
from(
select row_number() over() as rn
-- 	,(count(*) over())/2 as step_size
	,regexp_split_to_array(unnest(rowss),'') as entries
from(
select 
	(regexp_split_to_array(regexp_split_to_table(inputs,'\n'),' ')) as rowss
from temp_input
)t)t2
)
...
select rn
	...
	,letters
	,count(*)
	,count(1) over (partition by rn,wn) as num_letters
from words
-- where rn=35
group by 1,2,3
)
...
inner join counts c2
	on c.rn = c2.rn
	and c.wn <> c2.wn
	and c.letters = c2.letters
	and c.count = c2.count
	and c.num_letters = c2.num_letters
group by 1,2,3,4
having count(*) = c.num_letters
)t
)


select count(distinct w.rn)
from words w
left join bad b
	using(rn)
where b.rn is null
...
from words
group by 1
having count(entries) = count(distinct entries)
)t
;